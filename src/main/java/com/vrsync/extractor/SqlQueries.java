...
public final class SqlQueries {
    ...
    static {
        ...
        MSSQL.put("precificados", desde -> "SELECT p.id AS id_produto,\n       0 AS id_loja,\n       CAST(GETDATE() AS DATE) AS data,\n       p.precovenda,\n       e.custocomimposto,\n       e.customediocomimposto,\n       e.custosemimposto,\n       e.customediosemimposto,\n       CASE WHEN e.custocomimposto > 0 AND p.precovenda > 0\n            THEN ROUND(((p.precovenda - e.custocomimposto) / p.precovenda) * 100, 2)\n            ELSE 0 END AS margem_bruta_pct,\n       CASE WHEN e.custocomimposto > 0\n            THEN ROUND(p.precovenda - e.custocomimposto, 2)\n            ELSE 0 END AS margem_bruta_rs,\n       CASE WHEN e.custosemimposto > 0 AND p.precovenda > 0\n            THEN ROUND(((p.precovenda - e.custosemimposto) / p.precovenda) * 100, 2)\n            ELSE 0 END AS margem_liquida_pct,\n       CASE WHEN e.custosemimposto > 0\n            THEN ROUND(p.precovenda - e.custosemimposto, 2)\n            ELSE 0 END AS margem_liquida_rs,\n       e.estoque\nFROM produto p\nINNER JOIN estoque e ON e.id_produto = p.id\nWHERE p.ativo = 1\n  AND p.precovenda > 0\n  AND e.custocomimposto > 0\n");
        ...
        POSTGRES.put("precificados", desde -> "SELECT pc.id_produto,\n       pc.id_loja,\n       CURRENT_DATE AS data,\n       pc.precovenda,\n       pc.custocomimposto,\n       pc.customediocomimposto,\n       pc.custosemimposto,\n       pc.customediosemimposto,\n       CASE WHEN pc.custocomimposto > 0 AND pc.precovenda > 0\n            THEN ROUND(((pc.precovenda - pc.custocomimposto) / pc.precovenda) * 100, 2)\n            ELSE 0 END AS margem_bruta_pct,\n       CASE WHEN pc.custocomimposto > 0\n            THEN ROUND(pc.precovenda - pc.custocomimposto, 2)\n            ELSE 0 END AS margem_bruta_rs,\n       CASE WHEN pc.custosemimposto > 0 AND pc.precovenda > 0\n            THEN ROUND(((pc.precovenda - pc.custosemimposto) / pc.precovenda) * 100, 2)\n            ELSE 0 END AS margem_liquida_pct,\n       CASE WHEN pc.custosemimposto > 0\n            THEN ROUND(pc.precovenda - pc.custosemimposto, 2)\n            ELSE 0 END AS margem_liquida_rs,\n       pc.estoque\nFROM public.produtocomplemento pc\nWHERE pc.id_loja = {LOJA_ID}\n  AND pc.precovenda > 0\n  AND pc.custocomimposto > 0\n");
    }
}
